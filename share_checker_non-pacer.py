...
filename = nowDatestamp + ' Non-Pacer Share Checker ' + metamaskAddress + '.txt'
...
def masterdata():
    ...
    for n in range(len(masterdataParsed)):
        ...
        if totalRaces[i] != 0 and breedType[i] != "Pacer":
            ...
            if breedType[i] == "Rare":
                scholarEarnings = ((((gold[i]  * 126) + (silver[i] * 53) + (bronze[i] * 31)) / totalRaces[i])) * (share[i]* .01)
                totalEarnings = ((gold[i]  * 243) + (silver[i] * 122) + (bronze[i] * 40)) / totalRaces[i]
                preferredShare = (4/totalEarnings)*100
                preferredSharePublic = (1/totalEarnings)*100
                goldEarnings = gold[i] * 126
                silverEarnings = silver[i] * 53
                bronzeEarnings = bronze[i] * 31
            elif breedType[i] == "Epic":
                scholarEarnings = ((((gold[i]  * 342) + (silver[i] * 142) + (bronze[i] * 86)) / totalRaces[i])) * (share[i]* .01)
                totalEarnings = ((gold[i]  * 729) + (silver[i] * 365) + (bronze[i] * 121)) / totalRaces[i]
                preferredShare = (4/totalEarnings)*100
                preferredSharePublic = (1/totalEarnings)*100
                goldEarnings = gold[i] * 342
                silverEarnings = silver[i] * 142
                bronzeEarnings = bronze[i] * 86
            elif breedType[i] == "Legendary" or breedType[i] == "Founding":
                scholarEarnings = ((((gold[i]  * 2187) + (silver[i] * 1094) + (bronze[i] * 364)) / totalRaces[i])) * (share[i]* .01)
                totalEarnings = ((gold[i]  * 930) + (silver[i] * 388) + (bronze[i] * 232)) / totalRaces[i]
                preferredShare = (4/totalEarnings)*100
                preferredSharePublic = (1/totalEarnings)*100
                goldEarnings = gold[i] * 930
                silverEarnings = silver[i] * 388
                bronzeEarnings = bronze[i] * 232
            if totalRaces[i] > 200:
                accuracy = "Accurate"
            else:
                accuracy = "Inaccurate"
